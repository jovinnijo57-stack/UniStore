...
def get_user_active_token(email):
    try:
        ...
        return None
def update_wallet_balance(email, amount):
    ...
    try:
        ...
        if user_row:
            ...
            cursor.execute("UPDATE user_profiles SET wallet_balance = ? WHERE user_id = ?", (amount, uid))
            conn.commit()
            conn.close()
            return True
        conn.close()
        ...
        print(f"Error updating wallet balance in DB: {e}")
        return False
